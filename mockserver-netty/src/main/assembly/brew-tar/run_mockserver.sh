...
function showUsage {
    echo >&2 "   mockserver [-logLevel <level>] [-serverPort <port>] [-proxyPort <port>] [-proxyRemotePort <port>] [-proxyRemoteHost <hostname>] [-corsHeaders <header>]"
    echo >&2 "                                                                                   "
...
runCommand "java -Dfile.encoding=UTF-8 -Dmockserver.logLevel=$LOG_LEVEL $MOCK_OPTS -Dlog.dir=/usr/local/var/log/mockserver/ -jar /usr/local/lib/mockserver/mockserver-netty-jar-with-dependencies.jar$COMMAND_LINE_OPTS"